#ifndef WRAP_ISTEAMUSER_H
#define WRAP_ISTEAMUSER_H
#include "steam_api.h"
class ISteamUser_ 
{
  ...
  virtual void TrackAppUsageEvent( CGameID gameID, int eAppUsageEvent, const char *pchExtraInfo = "" ) __attribute__((thiscall));
  ...
  virtual EVoiceResult DecompressVoice( const void *pCompressed, uint32 cbCompressed, void *pDestBuffer, uint32 cbDestBufferSize, uint32 *nBytesWritten, uint32 nDesiredSampleRate ) __attribute__((thiscall));
  ...
  virtual SteamAPICall_t RequestStoreAuthURL( const char *pchRedirectURL ) __attribute__((thiscall));
