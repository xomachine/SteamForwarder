...
proc toDeclaration(args: seq[Arg]): string =
  args.map(to_declaration).join(", ")

proc makeHead(self: CallInfo): string =
  let fixed = fixcall(self)
  let name = if self.class.len > 0: self.class & "::" & self.name
    else: self.name & '_'
...
proc makeDeclaration*(self: CallInfo): string {.procvar.} =
  ...
  "$1 $2 $3($4);" % [virtual, fixed.returntype.to_declaration(), self.name,
                       fixed.args.to_declaration()]

proc makeTraceArgs(self: CallInfo): string =
  let args =
    if self.class.len > 0:
      (name: "this", thetype: (base: self.class, reference: "*")) & self.args
    else: self.args
  ...
  """TRACE("($1)"$2);""" % [format_args, final_trace_args]
...
proc makeTest*(self: CallInfo): string {.procvar.} =
  ...
  let tracer = self.makeTraceArgs()[7..^1]
  let arglist = self.args.mapIt(it.name).join(", ")
